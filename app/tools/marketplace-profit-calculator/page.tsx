...
export default function MarketplaceProfitCalculatorPage() {
  return <main className="resource-page">
    ...
    <section className="resource-content shell"><SellerEconomicsCalculator purchaseHref={purchaseHref} purchaseApiUrl={purchaseApiUrl} basePath={basePath} heading="Price the SKU with the full cost picture." intro="Start with a broad marketplace example, then enter the actual numbers for your category, fulfilment method and advertising plan." /></section>
    <section className="platform-related-section"><div className="shell"><div><span>Platform-specific calculators</span><h2>Start closer to the marketplace you sell on.</h2></div><div className="platform-related-links"><a href={`${basePath}/tools/amazon-seller-profit-calculator-india/`}>Amazon seller profit calculator <span>→</span></a><a href={`${basePath}/tools/flipkart-seller-profit-calculator-india/`}>Flipkart seller profit calculator <span>→</span></a><a href={`${basePath}/tools/meesho-profit-calculator/`}>Meesho profit calculator <span>→</span></a></div></div></section>
  </main>;
}
